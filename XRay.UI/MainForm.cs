...
namespace XRay.UI
{
    public partial class MainForm : Form
    {
        ...
        private List<ToolStripButton> tsbGroup = new List<ToolStripButton>();
        #endregion
        ...
        {
            tsbGroup.Add(tsbPatientCard);
            tsbGroup.Add(tsbRentgenAnalysis);
            tsbGroup.Add(tsbOutputInfo);
            tsbPatientCard.Checked = true;
            tsbButtonGroup_Click(tsbPatientCard, null);
        }

        #endregion

        private void tsbButtonGroup_Click(object sender, EventArgs e)
        {
            foreach (ToolStripButton b in tsbGroup)
            {
                if (sender != null && sender != b)
                {
                    b.Checked = false;
                }
            }

            tsbButtonGroup_HandleClick(sender, e);
        }

        private void tsbButtonGroup_HandleClick(object sender, EventArgs e)
        {
            tcPatientCard.TabPages.Clear();
            if (sender == tsbPatientCard)
            {
                tcPatientCard.TabPages.Add(tp1_GeneralInfo);
                tcPatientCard.TabPages.Add(tp1_DentalStatus);
            }
            else if (sender == tsbRentgenAnalysis)
            {
                tcPatientCard.TabPages.Add(tp2_Lines);
                tcPatientCard.TabPages.Add(tp2_AnatomicCharacteristics);
                tcPatientCard.TabPages.Add(tp2_RetensionCharacteristics);
            }
            else if (sender == tsbOutputInfo)
            {
                tcPatientCard.TabPages.Add(tp3_SurgeryCharacteristics);
                tcPatientCard.TabPages.Add(tp3_Recommendations);
            }
        }
    }
}
